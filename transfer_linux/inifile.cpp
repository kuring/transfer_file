...
#define MAX_INI_FILE_SIZE 1024*26
...
string IniFile::read(const string &key,const string &default_value) const
{
	char buf[300]={0};

	read_profile_string(m_section.c_str(),key.c_str(),buf,300,default_value.c_str(),m_fileName.c_str());
...
int GetPrivateProfileInt ( const char* section, const char* key,
	...
	return ini.read_profile_int(section,key,default_value,path);
}
...
bool WritePrivateProfileString(const char* section, const char* key,
	...
	IniFile ini(path);
	if (ini.write_profile_string(section,key,value,path) == 0)
	{
		return false;
	}
	return true;
...
int GetPrivateProfileString ( const char* section, const char* key, 
	 const char* default_value, char* value, int buflen, const char* path ) {
	if ( access( path, 0 ) != 0 )
		strncpy( value, default_value, buflen);

	IniFile ini(path);
	return ini.read_profile_string(section,key,value,buflen,default_value,path);
}
					...
					&& !end_of_string(p[i]) ) {
						...
						if(';' != p[j]) //skip over comment
						{
							while(j < i && p[j]!='=') {
								j++;
								if('=' == p[j]) {
									if((j - newline_start) == strlen(key) &&
											strncmp(key,p+newline_start,j-newline_start) == 0)
									{
										//find the key ok
										*key_s = newline_start;
										*key_e = j-1;
